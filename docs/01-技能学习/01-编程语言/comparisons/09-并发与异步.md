...
- 取消后仍要等待其他 worker settle，不能让函数返回时后台工作还在运行；
...
**并发、并行和异步有什么区别？** 并发是多个任务生命周期重叠，并行是多个 CPU 工作同时执行，异步是在等待时让出执行权；三者可以组合但不是同义词。

**发出取消后任务是否已经停止？** 没有。取消通常是协作信号，底层 IO、循环和等待点必须观察它，父任务还要等待子任务真正退出。

**为什么 goroutine、Promise、虚拟线程很轻仍要限流？** 任务本身变便宜，但连接、内存、队列、数据库和下游容量仍然有限。

## 语言内详细学习

- [C++：并发与异步](../languages/cpp/10-并发与异步.md)
- [Go：并发与异步](../languages/go/10-并发与异步.md)
- [Java：并发与异步](../languages/java/10-并发与异步.md)
- [Node.js / TypeScript：并发与异步](../languages/nodejs/10-并发与异步.md)
- [Python：并发与异步](../languages/python/10-并发与异步.md)
