...
**有 GC 为什么还会泄漏？** 仍可达对象不会回收；文件、连接、线程和任务还遵循各自的关闭协议，GC 不知道业务何时不再需要它们。

**RAII、defer、try-with-resources、finally 和 with 的共同点是什么？** 都把清理绑定到明确的作用域或控制流出口，让正常、错误和提前返回路径执行同一释放逻辑。

**怎样定位内存增长？** 先区分语言堆、native/Buffer、缓存和未关闭资源，再使用保留链、分配 profile 和进程指标找证据。

## 语言内详细学习

- [C++：内存与资源](../languages/cpp/08-内存与资源.md)
- [Go：内存与资源](../languages/go/08-内存与资源.md)
- [Java：内存与资源](../languages/java/08-内存与资源.md)
- [Node.js / TypeScript：内存与资源](../languages/nodejs/08-内存与资源.md)
- [Python：内存与资源](../languages/python/08-内存与资源.md)
