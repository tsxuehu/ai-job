...
```text
多态：调用方知道要调用 pay，运行时选择哪个实现
反射：程序在运行时询问“这是什么类型、有哪些成员”，再读取或调用
序列化/依赖注入：可以使用反射实现，但也可以使用代码生成或显式注册
```
...
| 语言 | 运行时入口 | 能否通用枚举字段 | 动态调用方式 | 最重要的限制 |
| --- | --- | --- | --- | --- |
| C++ | `typeid`、`dynamic_cast` | 标准 C++ 不支持 | 已知虚函数、显式注册表或第三方方案 | RTTI 只能做有限类型识别，不是完整反射 |
| Go | interface → `reflect.Type/Value` | 可枚举 struct 字段和 tag | `MethodByName`、`Call` | 未导出字段、nil、Kind、CanSet 容易导致错误或 panic |
| Java | `Class<?>` | 可查看字段、方法、构造器和运行时注解 | `Method.invoke`、`Constructor.newInstance` | 访问控制、模块、类型擦除和 AOT 会限制使用 |
| Node.js/TS | JavaScript 对象、`Object`、`Reflect` | 可查看对象和原型属性 | 取得函数后调用，或使用 `Proxy` 拦截 | TypeScript type/interface 已擦除，动态读取可能执行 getter |
| Python | `type`、`vars`、`dir`、`inspect` | 通常可以，但受对象模型影响 | `getattr` 后检查 `callable` 并调用 | descriptor、`__getattr__` 和动态名称会隐藏实际执行路径 |

同样是“按名字读取订单编号”，代码表面可以写成：

```cpp
// C++ 没有通用的 FieldByName("id")；通常写显式访问器
std::cout << order.id();
```

```go
id := reflect.ValueOf(order).FieldByName("ID").String()
```

```java
var field = order.getClass().getDeclaredField("id");
// private 字段还要满足访问规则；领域代码应优先调用 order.id()
```

```ts
const id = Reflect.get(order, "id");
```
...
这些代码只展示入口，不代表都应该在业务代码中使用。固定的订单模型应优先写普通访问器和接口，让编译器或类型检查器尽早发现错误。序列化器、ORM、测试工具、插件系统和框架容器面对未知类型时，反射才更有价值。

选择顺序可以固定为：

1. 类型和调用关系已知：普通字段、方法、interface/Protocol/抽象类；
2. 类型集合已知但需要分支：类型断言、type switch、sealed/variant 或显式映射；
3. 框架必须处理未知用户类型：集中使用反射，并把失败转换成明确错误；
4. 反射影响启动、AOT、类型安全或热点性能：考虑代码生成和编译期注册。

## 4. C++：本地代码，成本和生命周期直接可见
...
## 7. Node.js / TypeScript：事件循环最怕被 CPU 工作阻塞
...
## 8. Python：解释器成本、GIL 与 native 依赖
...
**多态和反射有什么区别？** 多态是在已知合同上选择具体实现，例如调用 `gateway.pay()`；反射是在运行时取得类型和成员元数据，再按名称读取或调用。前者通常保留更多编译期检查，后者更灵活但错误更晚暴露。

**为什么 C++ 的 RTTI 不等于 Java 反射？** RTTI 主要识别多态对象的实际类型并支持安全向下转换；标准 C++ 不能通用枚举任意类的字段和方法，也不能按字符串直接调用任意成员。
